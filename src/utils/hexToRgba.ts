...
export function validRGBA(rgba: Rgba | string) {
  if (rgba.length > 3) {
    ...
    else if (!(Number(rgba[0]) && Number(rgba[0]) >= 0 && Number(rgba[0]) <= 255)) {
      return false
    }
    //green value validation check
    else if (!(Number(rgba[1]) && Number(rgba[1]) >= 0 && Number(rgba[1]) <= 255)) {
      return false
    }
    //blue value validation check
    else if (!(Number(rgba[2]) && Number(rgba[2]) >= 0 && Number(rgba[2]) <= 255)) {
      return false
    }
...
export function RGBA2HEX(rgba: Rgba) {
  ...
  rgba.slice(0, 3).forEach((c, index) => {
    hex += parseInt(c).toString(16).length != 1 ? parseInt(c).toString(16) : '0' + parseInt(c).toString(16)
  })
  //alpha value change
  if (rgba[3]) {
...
export function getHexToRgba(hover: Params['hover']) {
  if (hover) {
    if (validRGBA(hover.color)) {
      ...
      console.log(HEX2RGBA(hover.color).join(','))
