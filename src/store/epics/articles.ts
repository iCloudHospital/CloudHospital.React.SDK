  ...
  action$.pipe(
    ...
    switchMap((action) =>
      from(apis.articles.loadArticles(action.payload)).pipe(
        ...
        catchError((restException: RestException) => of(loadArticlesAsync.failure(restException)))
      )
    )
  )
  ...
  action$.pipe(
    ...
    switchMap((action) =>
      from(apis.articles.loadArticles(action.payload)).pipe(
        ...
        catchError((restException: RestException) => of(appendArticlesAsync.failure(restException)))
      )
    )
  )
  ...
  action$.pipe(
    ...
    switchMap((action) =>
      from(apis.articles.loadArticle(action.payload)).pipe(
        ...
        catchError((restException: RestException) => of(loadArticleAsync.failure(restException)))
      )
    )
  )

export const loadTranslatedArticleEpic: RootEpic = (action$, state$, { apis }) =>
  action$.pipe(
    filter(isActionOf(loadTranslatedArticleAsync.request)),
    switchMap((action) =>
      from(apis.articles.loadArticle(action.payload)).pipe(
        map(loadTranslatedArticleAsync.success),
        catchError((restException: RestException) => of(loadTranslatedArticleAsync.failure(restException)))
      )
    )
  )
// #endregion Articles

const articlesEpic = combineEpics(loadArticlesEpic, appendArticlesEpic, loadArticleEpic, loadTranslatedArticleEpic)
