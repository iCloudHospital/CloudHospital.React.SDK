...
import {
  azSearchAutocompleteAsync,
  getSearchCountAsync,
  searchDealsAsync,
  searchDoctorsAsync,
  searchHospitalsAsync,
  searchSpecialtiesAsync,
  searchSpecialtyTypesAsync
} from '../actions/azSearch'
  ...
  action$.pipe(
    ...
    switchMap((action) =>
      from(apis.azSearch.azSearchAutocomplete(action.payload)).pipe(
        map(azSearchAutocompleteAsync.success),
        catchError((restException: RestException) => of(azSearchAutocompleteAsync.failure(restException)))
      )
    )
  )
  ...
  action$.pipe(
    ...
    switchMap((action) =>
      from(apis.azSearch.getSearchCount(action.payload)).pipe(
        map(getSearchCountAsync.success),
        catchError((restException: RestException) => of(getSearchCountAsync.failure(restException)))
      )
    )
  )
